...
function normalizeLines(value: unknown): MapLine[] {
  ...
  return value.filter((line): line is Record<string, unknown> => !!line && typeof line === 'object').map((line) => ({
    ...
    startTime: Math.max(0, Number(line.startTime ?? 0)),
...
export function rowToMap(row: MapRow): TypingMap {
  return {
    ...
    tags: row.tags ?? [],
    visibility: row.visibility as TypingMap['visibility'],
...
export function mapToRow(map: TypingMap, authorId: string) {
  return {
    ...
    youtube_video_id: map.youtubeVideoId,
    tags: map.tags,
...
export async function getPublicMaps(query = ''): Promise<TypingMap[]> {
  ...
  if (!supabase) return [];
  const { data, error } = await supabase.from('maps').select('*').in('visibility', ['public', 'unlisted']).order('updated_at', { ascending: false }).limit(100);
...
export async function recordPlay(mapId: string, accuracy: number, missCount: number, kpm: number): Promise<boolean> {
  ...
  const { error } = await supabase.rpc('record_play', { p_map_id: mapId, p_accuracy: accuracy, p_miss_count: missCount, p_kpm: kpm });
  return !error;
}
...
export async function toggleFavorite(mapId: string): Promise<{ ok: boolean; favorite: boolean }> {
  ...
  if (existing) {
    const { error } = await supabase.from('favorites').delete().eq('user_id', user.id).eq('map_id', mapId);
    return { ok: !error, favorite: false };
  }
