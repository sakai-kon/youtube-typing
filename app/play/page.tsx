...
export default function PlayPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (mapId === demoMap.id && alive) setMap(demoMap);
  ...
  if (!map) return <main className="container section"><div className="empty"><div className="empty-icon">⌁</div><h2>譜面が見つかりません</h2><p>公開譜面はSupabaseから読み込みます。ローカル譜面は作成したブラウザから開いてください。</p><Link className="btn" href="/">トップへ戻る</Link></div></main>;
  ...
  return <>
    <header className="site-header"><div className="container header-inner"><Link className="brand" href="/"><span className="brand-mark">YT</span><span>YouTube <b>Typing</b></span></Link><nav className="nav"><Link href="/create">譜面を作る</Link><Link href="/">終了</Link></nav></div></header>
    <main className="container section play-page">
      <div className="toolbar play-toolbar"><div><p className="eyebrow">NOW PLAYING</p><h1 className="page-title">{map.title}</h1><span className="muted">{finished ? lines.length : activeIndex + 1} / {lines.length} 行</span></div><span className="spacer"/><button className={`btn ${favorite ? 'favorite-active' : ''}`} onClick={handleFavorite}>{favorite ? '★ お気に入り済み' : '☆ お気に入り'}</button><button className="btn" onClick={()=>setReportOpen(v=>!v)}>通報</button></div>
      <div className="play-layout" style={{marginTop:14}}>
        <section><YouTubePlayer videoId={map.youtubeVideoId} onTime={setYtTime} compact /><div className="progress" style={{marginTop:10}}><div style={{width:`${Math.min(progress,100)}%`}}/></div><div className="play-metrics"><span>動画 {ytTime.toFixed(2)}s</span><span>ミス {misses}</span><span>KPM {kpm}</span><span>精度 {accuracy}%</span></div></section>
        <section className="play-stage"><div className="stage-top"><span className="live-dot"/> LIVE TYPING <span className="stage-hotkey">入力はページ内どこでもOK</span></div>{finished ? <><div className="complete-badge">COMPLETE</div><div className="play-text">おつかれさまでした！</div><p className="muted">今回のプレイ結果</p><div className="result"><div className="stat"><b>{misses}</b><span>ミス</span></div><div className="stat"><b>{kpm}</b><span>KPM</span></div><div className="stat"><b>{accuracy}%</b><span>精度</span></div><div className="stat"><b>{lines.length}</b><span>行数</span></div></div><div className="cta-row"><button className="btn primary" onClick={reset}>もう一度プレイ</button><Link className="btn" href="/">トップへ</Link></div></> : <><div className="line-progress">LINE {String(activeIndex + 1).padStart(2,'0')} / {String(lines.length).padStart(2,'0')}</div><div className="play-text">{current?.text || '—'}</div><div className="play-reading">{current?.reading || ''}</div><div className={`play-input ${lastKeyOk === false ? 'input-error' : lastKeyOk === true ? 'input-ok' : ''}`} aria-live="polite">{typed || 'キーボードで入力…'}</div><div style={{minHeight:22}}>{lastKeyOk === false ? <span className="error-inline">入力が違います</span> : lastKeyOk === true ? <span className="ok-inline">✓ 入力OK</span> : <span className="muted">Backspaceで1文字戻せます</span>}</div></>}</section>
      </div>
      {reportOpen && <section className="card report-panel" style={{marginTop:16}}><h2>譜面を通報</h2><form className="form" onSubmit={handleReport}><div className="field"><label>理由</label><select name="reason" required><option value="copyright">権利・著作権に関する問題</option><option value="inappropriate">不適切な内容</option><option value="spam">スパム</option><option value="other">その他</option></select></div><div className="field"><label>詳細</label><textarea name="details" maxLength={5000} placeholder="問題の内容を入力してください" /></div><button className="btn primary" disabled={!loggedIn}>通報を送信</button>{!loggedIn && <p className="muted">通報にはログインが必要です。</p>}{reportMessage && <p className="notice">{reportMessage}</p>}</form></section>}
    </main>
  </>;
}
