...
export default function PlayPage() {
  ...
  return <>
    ...
    <main className="container section play-page">
      <div className="toolbar play-toolbar"><div><p className="eyebrow">NOW PLAYING</p><h1 className="page-title">{title}</h1><span className="muted">{count}</span></div><span className="spacer"/>{map && <><button className={`btn ${favorite ? 'favorite-active' : ''}`} onClick={handleFavorite}>{favorite ? '★ お気に入り済み' : '☆ お気に入り'}</button><button className="btn" onClick={()=>setReportOpen(v=>!v)}>通報</button></>}</div>
      <div className="play-layout" style={{marginTop:14}}>
        <section>{map ? <><YouTubePlayer videoId={map.youtubeVideoId} onTime={setYtTime} compact /><div className="progress" style={{marginTop:10}}><div style={{width:`${Math.min(progress,100)}%`}}/></div></> : <div className="play-stage"><div className="stage-top"><span className="live-dot"/> LIVE TYPING <span className="stage-hotkey">入力はページ内どこでもOK</span></div><div className="play-text">{mapId ? '譜面を読み込んでいます…' : 'プレイする譜面を選択してください'}</div><div className="play-reading"> </div><div className="play-input">まもなく開始します…</div></div>}<div className="play-metrics"><span>動画 {ytTime.toFixed(2)}s</span><span>ミス {misses}</span><span>KPM {kpm}</span><span>精度 {accuracy}%</span></div></section>
        <section className="play-stage"><div className="stage-top"><span className="live-dot"/> LIVE TYPING <span className="stage-hotkey">入力はページ内どこでもOK</span></div>{loading ? <><div className="line-progress">READY</div><div className="play-text">{mapId ? '譜面を読み込んでいます…' : 'プレイする譜面を選択してください'}</div><div className="play-reading"> </div><div className="play-input">{mapId ? 'まもなく開始します…' : '譜面を選択してプレイ開始'}</div></> : finished ? <><div className="complete-badge">COMPLETE</div><div className="play-text">おつかれさまでした！</div><p className="muted">今回のプレイ結果</p><div className="result"><div className="stat"><b>{misses}</b><span>ミス</span></div><div className="stat"><b>{kpm}</b><span>KPM</span></div><div className="stat"><b>{accuracy}%</b><span>精度</span></div><div className="stat"><b>{lines.length}</b><span>行数</span></div></div><div className="cta-row"><button className="btn primary" onClick={reset}>もう一度プレイ</button><Link className="btn" href="/">トップへ</Link></div></> : <><div className="line-progress">LINE {String(activeIndex + 1).padStart(2,'0')} / {String(lines.length).padStart(2,'0')}</div><div className="play-text">{current?.text || '—'}</div><div className="play-reading">{current?.reading || ''}</div><div className={`play-input ${lastKeyOk === false ? 'input-error' : lastKeyOk === true ? 'input-ok' : ''}`} aria-live="polite">{typed || 'キーボードで入力…'}</div><div style={{minHeight:22}}>{lastKeyOk === false ? <span className="error-inline">入力が違います</span> : lastKeyOk === true ? <span className="ok-inline">✓ 入力OK</span> : <span className="muted">Backspaceで1文字戻せます</span>}</div>}</section>
      </div>
