...
export default function PlayPage() {
  ...
  const reset = () => {
    ...
    setStartedAt(null); setFinished(false); setActiveIndex(0); setLastKeyOk(null);
  };
  ...
  return <>
    <header className="site-header"><Link className="brand" href="/"><span className="brand-mark">YT</span> YouTube <span>Typing</span></Link><nav className="nav"><Link href="/create">譜面を作る</Link><Link href="/">終了</Link></nav></header>
    <main className="container section play-page">
      <div className="toolbar play-toolbar">
        <div><p className="eyebrow">NOW PLAYING</p><h1 className="page-title">{map.title}</h1><span className="muted">{finished ? lines.length : activeIndex + 1} / {lines.length} 行</span></div>
        <span className="spacer"/>
        <button className={`btn ${favorite ? 'favorite-active' : ''}`} onClick={handleFavorite}>{favorite ? '★ お気に入り済み' : '☆ お気に入り'}</button>
        <button className="btn" onClick={()=>setReportOpen(v=>!v)}>通報</button>
      </div>
      <div className="play-layout" style={{marginTop:14}}>
        <section>
          <YouTubePlayer videoId={map.youtubeVideoId} onTime={setYtTime} compact />
          <div className="progress" style={{marginTop:10}}><div style={{width:`${Math.min(progress,100)}%`}}/></div>
