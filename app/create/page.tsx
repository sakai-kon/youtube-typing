...
export default function CreatePage() {
  ...
  const save = async () => {
    ...
    if (loggedIn) {
      ...
      setSaved(result.ok ? 'Supabaseに保存しました。' : `ローカルには保存しました：${result.error ?? '保存に失敗しました。'}`);
  ...
  return <>
    <header className="site-header"><Link className="brand" href="/">YouTube <span>Typing</span></Link><nav className="nav"><Link href="/search">検索</Link><Link href="/">トップ</Link>{loggedIn ? <Link href="/profile/me">プロフィール</Link> : <Link href="/login">ログイン</Link>}</nav></header>
