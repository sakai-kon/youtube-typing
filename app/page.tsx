    ...
    <main>
      <section className="hero container">
        <div className="hero-copy">
          <div className="eyebrow">YOUTUBE × TYPING</div>
          <h1>動画の「今」を、<br/><span>打ち込むゲーム。</span></h1>
          <p>YouTubeを再生しながら、自分で譜面を作る。あとはタイミングに合わせてローマ字で打つだけ。練習にも、遊びにも。</p>
          <div className="cta-row hero-actions"><Link className="btn primary btn-large" href="/search">すぐ遊ぶ <span>→</span></Link><Link className="btn btn-large" href="/create">譜面を作る</Link></div>
          <div className="hero-note"><span className="status-dot"/>ブラウザだけで動作　・　AI判定なし　・　入力コスト0</div>
        </div>
        <div className="hero-visual">
          <div className="mock-window"><div className="mock-top"><span/><span/><span/><em>typing stage</em></div><div className="mock-video"><div className="mock-play">▶</div></div><div className="mock-stage"><small>NOW PLAYING</small><strong>今日も一日頑張ろう</strong><span>kyoumoichinichiganbarou</span><div className="mock-input">kyoumoichi<span>▍</span></div></div></div>
        </div>
      </section>
      ...
      <section className="section container">
        <div className="section-heading"><div><p className="eyebrow">DISCOVER</p><h2>譜面を探す</h2></div><Link className="text-link" href="/search">すべて見る →</Link></div>
        <div className="toolbar browse-toolbar"><div className="search-shell"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="タイトル・タグを検索" aria-label="譜面を検索"/></div><span className="muted">{visible.length}件</span></div>
        {loading ? <div className="empty">譜面を読み込んでいます…</div> : <div className="grid map-grid">{visible.slice(0,6).map((map) => <article className="card map-card" key={map.id}><div className="card-topline"><span className="card-kicker">{map.visibility === 'public' ? 'PUBLIC' : 'LOCAL'}</span><span className="muted">{map.lines.length}行</span></div><h3>{map.title}</h3><p className="muted">{map.description || '説明なし'}</p><div className="tag-row">{map.tags.slice(0,4).map((tag) => <span className="tag" key={tag}>#{tag}</span>)}</div><div className="cta-row"><Link className="btn primary" href={`/play?id=${encodeURIComponent(map.id)}`}>プレイする</Link>{map.authorId === 'local' && <Link className="btn" href={`/create?edit=${encodeURIComponent(map.id)}`}>編集</Link>}</div></article>)}</div>}
        {!visible.length && !loading && <div className="empty"><h3>見つかりませんでした</h3><p>別のキーワードで検索してみてください。</p></div>}
      </section>
      ...
      <section className="section container"><div className="notice callout"><div><strong>まずはサンプル譜面で試せます。</strong><span>ログインなしでもローカル保存・JSON入出力・プレイができます。</span></div><button className="btn" onClick={seed}>サンプルを保存</button></div></section>
    </main>
    <footer className="footer"><div className="container footer-inner"><span>YouTube Typing</span><span>user-created typing maps</span></div></footer>
  </>;
}
