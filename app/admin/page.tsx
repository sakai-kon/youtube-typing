...
export default function AdminPage() {
  ...
  if (!allowed) return <><SiteHeader /><main className="container section"><div className="card"><p className="eyebrow">ADMIN</p><h1>管理画面</h1><div className="notice">{message}</div><div className="cta-row"><Link className="btn" href="/">トップへ戻る</Link><Link className="btn" href="/copyright">著作権申立てページ</Link></div></div></main></>;

  return <><SiteHeader /><main className="container section"><div className="page-head"><div><p className="eyebrow">ADMIN / MODERATION</p><h1 className="page-title">管理画面</h1><p className="muted">譜面の管理、ユーザー権限、著作権申立てをここから確認します。</p></div></div>{message && <p className="notice">{message}</p>}
    <CopyrightRequests />
